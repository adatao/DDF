...
import com.adatao.spark.ddf.analytics.util.DenseVector
import com.adatao.spark.ddf.analytics.util.SparseVector
import com.adatao.spark.ddf.analytics.util.Vector
import com.adatao.spark.ddf.analytics.util.Vectors
import com.adatao.spark.ddf.analytics.util.{DenseVector, SparseVector, Vector, Vectors}
import com.adatao.spark.ddf.analytics.RDDFunctions._
...
class IDFModel (val idf: Vector) extends Serializable {
  ...
  /**
   * Transform term frequency (TF) vectors to TF-IDF vectors
   * @param data a term frequency vectors
  ...
  def transform(data: Vector): Vector = {
    ...
    data match {
      case sv: SparseVector => {
        ...
        Vectors.sparse(size, sv.indices, newValues)
      }
      case dv: DenseVector => {
