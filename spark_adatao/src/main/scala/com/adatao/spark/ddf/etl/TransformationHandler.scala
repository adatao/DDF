...
class TransformationHandler(mDDF: DDF) extends THandler(mDDF) {
  ...
  def dummyCoding(xCols: Array[String], yCol: String): SparkDDF = {
    ...
    val rddMatrixVector: RDD[TupleMatrixVector] = null //TransformDummy.getDataTable(tp, xColsIndex, yColIndex, categoricalMap)
    ...
    if (hasDummyCoding) {
      //TODO: migrate to SPARKSQL
      val rddMatrixVector2: RDD[TupleMatrixVector]  = null // rddMatrixVector.map(TransformDummy.instrument(xColsIndex, categoricalMap))
