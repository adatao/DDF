...
/**
 * <p>
 * Abstract base class for a {@link DDF} implementor, which provides the support
 * methods necessary to implement various DDF interfaces, such as
 * {@link IHandleRepresentations} and {@link ISupportML}.
 * </p>
 * <p>
 * We use the Dependency Injection, Delegation, and Composite patterns to make
 * it easy for others to provide alternative (even snap-in replacements),
 * support/implementation for DDF. The class diagram is as follows:
 * </p>
 * 
 * <pre>
 * ------------------   -------------------------
 ...
 * </pre>
 * <p>
 * An implementor need not provide all or even most of these interfaces. Each
 * interface handler can be get/set separately, as long as they cooperate
 * properly on things like the underlying representation. This makes it easy to
 * roll out additional interfaces and their implementations over time.
 * </p>
 * DDFManager implements {@link IHandleSqlLike} because we want to expose those
 * methods as directly to the API user as possible, in an engine-dependent
 * manner.
 * 
 * @author ctn
 * 
 */
public abstract class DDFManager extends ALoggable implements IDDFManager,
		IHandleSqlLike, ISupportPhantomReference {
	...
	//lookup name (name, aliasName)
	protected Map<String, String> mLookUpName = new HashMap<String, String>();
	...
	public static DDFManager get(String engineName) throws DDFException {
		...
		String className = Config.getValue(engineName,
				ConfigConstant.FIELD_DDF_MANAGER);
	...
	public DDF newDDF(Object data, Class<?>[] typeSpecs, String namespace,
	...
	/**
	 * Instantiates a new DDF of the type specified in ddf.ini as "DDF", using
	 * the constructor that requires only {@link DDFManager} as an argument.
	 * 
	 * @param manager
	...
	public DDF newDDF(DDFManager manager) throws DDFException {
		return this.newDDF(new Class<?>[] { DDFManager.class },
				new Object[] { manager });
	}

	/**
	 * Instantiates a new DDF of the type specified in ddf.ini as "DDF", passing
	 * in this DDFManager as the sole argument
	 * 
	 * @return the newly instantiated DDF
	 * @throws DDFException
	 */
	...
	private DDF newDDF(Class<?>[] argTypes, Object[] argValues)
			throws DDFException {

		String className = Config.getValueWithGlobalDefault(this.getEngine(),
				ConfigConstant.FIELD_DDF);
		if (Strings.isNullOrEmpty(className))
			throw new DDFException(String.format(
					"Cannot determine class name for [%s] %s",
					this.getEngine(), "DDF"));

		try {
			Constructor<DDF> cons = (Constructor<DDF>) Class.forName(className)
					.getDeclaredConstructor(argTypes);
			if (cons == null)
				throw new DDFException("Cannot get constructor for "
						+ className);
			...
			if (ddf == null)
				throw new DDFException("Cannot instantiate a new instance of "
						+ className);
			...
			throw new DDFException(
					String.format(
							"While instantiating a new %s DDF of class %s with argTypes %s and argValues %s",
							this.getEngine(), className,
	...
	public String getNamespace() throws DDFException {
		if (Strings.isNullOrEmpty(mNamespace)) {
			mNamespace = Config.getValueWithGlobalDefault(this.getEngine(),
					ConfigConstant.FIELD_NAMESPACE);
		}
	...
	@Override
	public DDF sql2ddf(String command, Schema schema, String dataSource)
			throws DDFException {
	...
	@Override
	public List<String> sql2txt(String command, Integer maxRows)
			throws DDFException {
	...
	public List<String> sql2txt(String command, Integer maxRows,
		...
		return this.getDummyDDF().getSqlHandler()
				.sql2txt(command, maxRows, dataSource);
	}
	...
	public static IPersistible doLoad(PersistenceUri uri) throws DDFException {
		if (uri == null)
			throw new DDFException("URI cannot be null");
	...
	public abstract DDF loadTable(String fileURL, String fieldSeparator)
			throws DDFException;
}
