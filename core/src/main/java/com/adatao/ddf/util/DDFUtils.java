...
public class DDFUtils {
  ...
  private static String generateUniqueName(Object obj) {
    ...
    if (obj instanceof DDF) {
      return String.format("%s-%s-%s", obj.getClass().getSimpleName(), ((DDF) obj).getEngine(), UUID.randomUUID());
    }

    return String.format("%s-%s", obj.getClass().getSimpleName(), UUID.randomUUID());
  }
