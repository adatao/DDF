...
class RepresentationHandler(mDDF: DDF) extends RH(mDDF) {
  ...
  override def createRepresentation(typeSpecs: String): Object = {
    ...
    if (this.has(RDD_ROW) && CanBeCreatedFrom(RDD_ROW).contains(typeSpecs)) {
      this.fromRDDRow(typeSpecs)

    } else if (this.has(RDD_ARRAY_OBJECT) && CanBeCreatedFrom(RDD_ARRAY_OBJECT).contains(typeSpecs)) {
      //TODO
      null
    } else if (this.has(RDD_ARRAY_DOUBLE) && CanBeCreatedFrom(RDD_ARRAY_DOUBLE).contains(typeSpecs)) {
      //TODO
      null
    } else if (this.has(RDD_LABELED_POINT) && CanBeCreatedFrom(RDD_LABELED_POINT).contains(typeSpecs)) {
      //TODO
      null
    }

    else {
      throw new DDFException("Cannot create representation: " + typeSpecs)
    }
  }

  override def canCreate(typeSpecs: String): Boolean = {
    this.getExistingRepresentations.exists(representation => CanBeCreatedFrom(representation).contains(typeSpecs))
  }
...
object RepresentationHandler {
  ...
  val CanBeCreatedFrom: Map[String, List[String]] = HashMap(
    RDD_ROW -> List(RDD_ARRAY_DOUBLE, RDD_ARRAY_OBJECT, RDD_LABELED_POINT),
    RDD_ARRAY_DOUBLE -> List(RDD_ARRAY_OBJECT, RDD_LABELED_POINT, RDD_ROW),
    RDD_ARRAY_OBJECT -> List(RDD_ARRAY_DOUBLE, RDD_LABELED_POINT, RDD_ROW),
    RDD_LABELED_POINT -> List(RDD_ARRAY_DOUBLE, RDD_ARRAY_OBJECT, RDD_ROW)
  )
