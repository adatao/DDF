...
class JBlasSuite extends ATimedAlgorithmTest {
	...
	test("Automatic Java Matrix-Vector [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, 1, REPS)) {
		(1 to REPS).foreach({ i ⇒ a.mmul(x) })
	}

	test("BLAS Level 2: SimpleBlas.gemv Matrix-Vector [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, 1, REPS)) {
		(1 to REPS).foreach({ i ⇒ SimpleBlas.gemv(alpha, a, x, beta, y) })
	}

	test("BLAS Level 2: NativeBlas.dgemv Matrix-Vector [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, 1, REPS)) {
		(1 to REPS).foreach({ i ⇒ NativeBlas.dgemv('N', a.rows, a.columns, alpha, a.data, 0, a.rows, x.data, 0, 1, beta, y.data, 0, 1) })
	}

	test("BLAS Level 3: NativeBlas.dgemm Matrix-Vector [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, 1, REPS)) {
		val c = y
	...
	test("Automatic Java Matrix-Matrix [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, K, REPS)) {
	  val x = x2
		val y = y2
		(1 to REPS).foreach({ i ⇒ a.mmul(x) })
	}

	test("BLAS Level 3: NativeBlas.dgemm Matrix-Matrix [%dx%d]x[%dx%d] multiplication, %d reps".format(M, N, N, K, REPS)) {
	  val x = x2
		val y = y2
		val c = y
		(1 to REPS).foreach({ i ⇒ NativeBlas.dgemm('N', 'N', c.rows, c.columns, a.columns, alpha, a.data, 0, a.rows, b.data, 0, b.rows, beta, c.data, 0, c.rows) })
