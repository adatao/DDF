...
/*
	ignore("transform scale standard on file") {
...
//		assert(summary.result.stdev.forall(x => math.abs(x - 1) < 0.02))
		...
		val scaleres2 = bigRClient.execute[com.adatao.pa.spark.Utils.DataFrameResult](scaler2)
...
/*
	ignore("transform scale preserving factors") {
		createTableMtcars
