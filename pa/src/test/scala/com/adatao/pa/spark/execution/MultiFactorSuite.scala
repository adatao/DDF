...
class MultiFactorSuite extends ABigRClientTest {
  ...
  test("test Shark") {
    ...
    val result = bigRClient.execute[Array[(Int, JMap[String, java.lang.Double])]](cmd).result
...
//    val dcID = this.loadFile(List("resources/mtcars", "server/resources/KmeansTest.csv"), false, " ")
    ...
    assert(result(0)._2.get("1") == 14.0)
    assert(result(0)._2.get("0") == 18.0)
    assert(result(1)._2.get("1") == 13.0)
    assert(result(2)._2.get("4") == 12.0)
    assert(result(2)._2.get("3") == 15.0)
    assert(result(2)._2.get("5") == 5.0)
    assert(result(3)._2.get("1") == 7.0)
    assert(result(3)._2.get("2") == 10.0)
    assert(result(3)._2.get("8") == 1.0)
  }
  test("test NA handling") {
    ...
    val result2 = bigRClient.execute[Array[(Int, JMap[String, java.lang.Double])]](cmd2).result
