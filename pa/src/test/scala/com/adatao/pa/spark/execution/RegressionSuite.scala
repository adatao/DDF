...
//
  ...
  test("Multiple-variable logistic regression IRLS - no regularization") {
    ...
    assertEquals(truncate(model.weights(0), 6), -21.584582, 0.1)
    assertEquals(truncate(model.weights(1), 6), 0.221777, 0.05)
    assertEquals(truncate(model.weights(2), 6), 0.203507, 0.05)
    assertEquals(truncate(model.deviance, 6), 32.416312, 0.1)
    //assert(truncate(model.nullDeviance, 6) === 68.0292)
    assertEquals(truncate(model.stderrs(0), 6), 6.417354, 0.1)
    assertEquals(truncate(model.stderrs(1), 6), 0.074349, 0.01)
    assertEquals(truncate(model.stderrs(2), 6), 0.062723, 0.01)
    assert(model.numFeatures == 2)
