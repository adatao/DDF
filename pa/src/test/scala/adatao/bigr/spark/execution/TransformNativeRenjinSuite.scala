...
class TransformNativeRenjinSuite extends ABigRClientTest {
	...
	test("can add column") {
		...
		val transformer = new TransformNativeRenjin(dataContainerId, "newcol = mpg / gear")
	...
	test("can add multiple columns on data w some empty partitions") {
		...
		val transformer = new TransformNativeRenjin(dataContainerId,
			"z1 = mpg / gear, " +
			"z2 = mpg * 0.4251437075, " +
	...
	test("can update column values") {
		...
		val transformer = new TransformNativeRenjin(dataContainerId, "mpg = mpg * 0.4251437075")
		val r1 = bigRClient.execute[DataFrameResult](transformer)
		assert(r1.isSuccess)
