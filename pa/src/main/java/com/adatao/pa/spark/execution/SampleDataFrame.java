...
public class SampleDataFrame implements IExecutor {
  ...
  public ExecutorResult run(SparkThread sparkThread) throws AdataoException {
    ...
    if (getPercent) {
      /*
       * JavaRDD<Object[]> data = dataTable.sample(replace, percent, seed); DataFrame tdf; tdf = new
       * DataFrame(df.getMetaInfo(), data); JavaSharkContext jsc = (JavaSharkContext) sparkThread.getSparkContext();
       * SharkDataFrame sdf= SharkUtils.createSharkDataFrame(tdf, jsc); String uid = dm.add(sdf); // TODO Auto-generated
       * method stub
       * 
      ...
      if (size > MAX_SAMPLE_SIZE) {
        throw new AdataoException(AdataoExceptionCode.ERR_SAMPLE_MAX_SIZE, String.format(
            "Sample size exceeds limit: size %d, MAX_SAMPLE_SIZE %d", size, MAX_SAMPLE_SIZE), null);
      }
      /*
        ...
        if (ddf != null && ddf.getSchema() != null) {
          try {
            ...
            throw new AdataoException(AdataoExceptionCode.ERR_GENERAL,
                "Fail to get sample data for dataframe: [MetaInfo] = " + metaInfos, null);
          } catch (DDFException e) {
            throw new AdataoException(AdataoExceptionCode.ERR_GENERAL, e.getMessage(), e.getCause());
          }
        }
