...
public class SampleDataFrame implements IExecutor {
	...
	static public class SampleDataFramePercentResult extends SuccessResult {
		...
		public SampleDataFramePercentResult setDataContainerID(
				String dataContainerID) {
			this.dataContainerID = dataContainerID;
			return this;
		}
	...
	static public class SampleDataFrameSizeResult extends SuccessResult {
		...
		public SampleDataFrameSizeResult setDataContainerID(
				String dataContainerID) {
			this.dataContainerID = dataContainerID;
			return this;
		}
	...
	public ExecutorResult run(SparkThread sparkThread) throws AdataoException {
		DDF ddf = (DDF) sparkThread.getDDFManager().getDDF(
				Utils.getDDFNameFromDataContainerID(dataContainerID));

		if (ddf == null) {
			throw new AdataoException(
					AdataoExceptionCode.ERR_DATAFRAME_NONEXISTENT,
					"Cannot find DDF " + dataContainerID, null);
		}
		...
		if (getPercent) {
			/*
			 * JavaRDD<Object[]> data = dataTable.sample(replace, percent,
			 * seed); DataFrame tdf; tdf = new DataFrame(df.getMetaInfo(),
			 * data); JavaSharkContext jsc = (JavaSharkContext)
			 * sparkThread.getSparkContext(); SharkDataFrame sdf=
			 * SharkUtils.createSharkDataFrame(tdf, jsc); String uid =
			 * dm.add(sdf); // TODO Auto-generated method stub
			 * 
			 * return new SampleDataFramePercentResult()
			 * .setDataContainerID(uid) .setMetaInfo(tdf.getMetaInfo());
			 */
			DDF sampleDDF = ddf.Views.getRandomSample(percent, replace, seed);
			try {
				MetaInfo[] metaInfos = Utils.generateMetaInfo(sampleDDF
						.getSchema());
			...
			int MAX_SAMPLE_SIZE = Integer.parseInt(System.getProperty(
					"sample.max.size", "10000000"));
			if (size > MAX_SAMPLE_SIZE) {
				throw new AdataoException(
						AdataoExceptionCode.ERR_SAMPLE_MAX_SIZE,
						String.format(
								"Sample size exceeds limit: size %d, MAX_SAMPLE_SIZE %d",
								size, MAX_SAMPLE_SIZE), null);
			}
			/*
			 * List<Object[]> data = dataTable.takeSample(replace, size, seed);
			 * 
			 * if(data != null && data.size() > 0) { return new
			 * SampleDataFrameSizeResult() .setDataContainerID(dataContainerID)
			 * .setData(data);
			 */
			List<Object[]> data = ddf.Views
					.getRandomSample(size, replace, seed);
			if (data != null && data.size() > 0) {
				return new SampleDataFrameSizeResult().setDataContainerID(
						dataContainerID).setData(data);
			} else {
				// provide metaInfo for easy debugging. At this step it's
				// certain that df is not null
				if (ddf != null && ddf.getSchema() != null) {
					try {
						MetaInfo[] metaInfos = Utils.generateMetaInfo(ddf
								.getSchema());
						throw new AdataoException(
								AdataoExceptionCode.ERR_GENERAL,
								"Fail to get sample data for dataframe: [MetaInfo] = "
										+ metaInfos, null);
					} catch (DDFException e) {
						throw new AdataoException(
								AdataoExceptionCode.ERR_GENERAL,
