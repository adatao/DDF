...
public class Subset extends CExecutor {
  ...
  static public class Operator extends Expr {
    ...
    public String toSql() {
      if (name == null) {
        throw new IllegalArgumentException("missing Operator name from BigR client for operands[] "
  ...
  public ExecutorResult run(SparkThread sparkThread) throws AdataoException {
    ...
    if (dc.getType() == DataContainer.ContainerType.DataFrame) {
      ...
      DataFrame subset;
      if (filter != null) {
        subset = new DataFrame(selectColumnMetaInfo(dc), table.filter(new FilterMapper(filter)).map(
            new ColumnSelectionMapper(columns)));
      } else {
