...
import com.adatao.pa.spark.execution.VectorHistogram.HistogramBin;
...
public class VectorHistogram implements IExecutor, Serializable {
  ...
  public ExecutorResult run(SparkThread sparkThread) {
    ...
    if (dc.getType().equals(DataContainer.ContainerType.SharkColumnVector)) {
      ...
      List<String> res = sc.sql(String.format("select histogram_numeric(%s,%s) from %s", v.getColumn(), numBins,
          v.tableName));
      ...
      JavaRDD<Object[]> filteredRdd = df.getRDD().filter(new Function<Object[], Boolean>() {
        @Override
        public Boolean call(Object[] t) {
          return (t[0] != null) && ((t[0] instanceof Integer) || (!Double.isNaN((Double) t[0])));
        }
      });
      NumericHistogram nh = filteredRdd.mapPartitions(new FlatMapFunction<Iterator<Object[]>, NumericHistogram>() {
        @Override
        public Iterable<NumericHistogram> call(Iterator<Object[]> arg0) {
          List<NumericHistogram> res = new ArrayList<NumericHistogram>(1);
        ...
        @Override
        public NumericHistogram call(NumericHistogram arg0, NumericHistogram arg1) {
      ...
      return new FailResult().setMessage("invalid data container type for operation: " + dc.getType());
    }
  }
}
