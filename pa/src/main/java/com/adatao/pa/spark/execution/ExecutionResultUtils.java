...
public class ExecutionResultUtils {
  ...
  /**
   * This function is used to process the message of SparkException returned from a failed job execution. Adatao patch
   * https://github.com/adatao/spark/pull/1 allows Spark to send back info about slaves' exceptions to master. This info
   * is embedded in the message of SparkException. The message has the following format:
   * 
   * <pre>
   * &quot;Job aborted: ExceptionFailure: ${slave exception}: ${slave exception message}&quot;
   * </pre>
   * 
   * If the slaves's exception is AdataoException. The message has the following format:
   * 
   * <pre>
   * &quot;Job aborted: ExceptionFailure: adatao.AdataoException: ${AdataoExceptionCode}: ${exception message}&quot;
   * </pre>
   * 
   * 
  ...
  public static AdataoException processException(Exception e) {
    ...
    if (e.getClass().getName() == "org.apache.spark.SparkException" && msgSplits.length >= 5
        && message.matches(adataoMsgPrefixRegex)) {
    ...
    } else if (e instanceof java.lang.UnsupportedOperationException && message.matches(emptyCollectionRegex)) {
