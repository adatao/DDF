...
public class SharkQueryUtils {
   ...
   * @param sc
   * @param cmd
   * @param adaExceptionCode
   *          : this is a incident-specific AdataoExceptionCode
   * @return an non-empty List<String>
   * @throws AdataoException
   *           with {@code adaExceptionCode} when {@code SharkContext} returns a
   *           {@code shark.api.QueryExecutionException}
   * @throws AdataoException
   *           with {@code ERR_SHARK_RESULT_EMPTY} when the query result is empty or there is one value "NULL"
   */
  public static List<String> sql2ListString(JavaSharkContext sc, String cmd, AdataoExceptionCode adaExceptionCode)
      throws AdataoException {
    try {
      List<String> res = sc.sql(cmd);
      if (res.size() == 0 || (res.size() > 0 && res.get(0).equals("NULL"))) {
        /**
         * this happens when data has NA values or there is not data, we return null
         */
  ...
  /**
   * This function will return a Double value or @{code null} when get exception with code
   * {@code ERR_SHARK_RESULT_EMPTY}
   * 
   * @param sc
   * @param cmd
   * @param adaExceptionCode
   *          : this is a incident-specific AdataoExceptionCode thrown when {@code SharkContext} returns a
   *          {@code shark.api.QueryExecutionException}
   * @return a Double value or null
   * @throws AdataoException
   *           with {@code adaExceptionCode} when {@code SharkContext} returns a
   *           {@code shark.api.QueryExecutionException}, or just throw other exceptions if any
   */
  public static Double sql2Double(JavaSharkContext sc, String cmd, AdataoExceptionCode adaExceptionCode)
      throws AdataoException {
    try {
   ...
   *           {@code shark.api.QueryExecutionException}, or just throw other exceptions if any
   */
  public static Long sql2Long(JavaSharkContext sc, String cmd, AdataoExceptionCode adaExceptionCode)
      throws AdataoException {
    try {
      List<String> res = sql2ListString(sc, cmd, adaExceptionCode);
      try {
