...
public class SparkThread extends ASessionThread {
  ...
  GsonBuilder gsonBld = new GsonBuilder().serializeSpecialFloatingPointValues().registerTypeAdapter(Subset.Expr.class,
      new Subset.ExprDeserializer());
  Gson gson = gsonBld.setExclusionStrategies(new ExclusionStrategy() {
    ...
    /**
     * We need to ignore Manifest type that occur in Scala classes (e.g., Manifest[LinearRegressionModel]), and we don't
     * know how to/don't need to deserialize them either for our purposes here.
     */
    @Override
    public boolean shouldSkipClass(Class<?> arg0) {
      if (scala.reflect.Manifest.class.equals(arg0)) return true;
      return false;
    }
  }).create();
  ...
  private void processJsonCommand(JsonCommand jsCmd) throws JsonSyntaxException, InterruptedException,
      ClassNotFoundException, AdataoException, DDFException {
  ...
  public ExecutionResult<?> processJsonCommand1(JsonCommand jsCmd) throws JsonSyntaxException, ClassNotFoundException,
      AdataoException, DDFException {
    Object exec = gson.fromJson(jsCmd.params, Class.forName("com.adatao.pa.spark.execution." + jsCmd.getCmdName()));
    LOG.info("Created Executor: " + exec.toString());
  ...
  private static Map<String, String> getEnvironment() {
    ...
    if (s != null && s.length() > 0) result.put("SPARK_CLASSPATH", s);

    s = System.getProperty("spark.master", System.getenv("SPARK_MASTER"));
    if (s != null && s.length() > 0) result.put("SPARK_MASTER", s);
    else {
      try {
    ...
    if (s != null && s.length() > 0) result.put("SPARK_HOME", s.replaceAll(" ", "%20"));

    s = System.getProperty("rserver.home", System.getenv("RSERVER_HOME"));
    if (s != null && s.length() > 0) result.put("RSERVER_HOME", s.replaceAll(" ", "%20"));

    s = System.getProperty("rserver.jar", System.getenv("RSERVER_JAR"));
    if (s != null && s.length() > 0) result.put("RSERVER_JAR", s.replaceAll(" ", "%20"));
  ...
  public JavaSparkContext startLocalSparkContext(Boolean isShark) {
    if (!isShark) return new JavaSparkContext("local[2]", "BigR");
    else return SharkEnv.initWithJavaSharkContext(new JavaSharkContext("local[2]", "BigR"));
  }
  ...
  public void run() {
    ...
    try {
      ...
      JsonResult res = new JsonResult()
          .setResult(new FailedResult<Object>(AdataoExceptionCode.ERR_GENERAL.getMessage()).toJson());
      try {
        resQueue.put(res);
