...
//    val diff12: VertexRDD[String] = vertices2.diff(vertices1)
    ...
    LOG.info(">>filteredGraph1 = " + filteredGraph1.vertices.collect().map{case (id, num) => println(">>> num= " + num)})
    LOG.info(">>filteredGraph2 = " + filteredGraph2.vertices.collect().map{case (id, num) => println(">>> num= " + num)})
